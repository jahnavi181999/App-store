...
class AppStore extends Component {
  state = {
    searchInput: '',
    userAppsList: appsList,
    initialtabsList: tabsList[0].tabId,
  }
  ...
  filterCategory = tabId => {
    this.setState({
      initialtabsList: tabId,
    })
  }

  getFilteredApps = () => {
    const {initialtabsList} = this.state
    const filteredApps = appsList.filter(
      eachApp => eachApp.category === initialtabsList,
    )
  ...
  render() {
    const {searchInput, userAppsList, initialtabsList} = this.state
          ...
          <ul className="category">
            {tabsList.map(eachCategory => (
              <TabItem
                ...
                value={eachCategory.tabId}
                filterCategory={this.filterCategory}
                isActive={initialtabsList === eachCategory.tabId}
              />
            ))}
          </ul>
          <div className="appsNames">
            <ul className="apps-unordered-list">
